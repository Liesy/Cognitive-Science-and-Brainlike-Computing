...
class S1(nn.Module):
    ...
    def __init__(self, size, wavelength, orientations=[90, -45, 0, 45]):
        ...
        self.gabor = nn.Conv2d(1, self.num_orientations, size, padding=size // 2, bias=False)
        # Fill the Conv2d filter weights with Gabor kernels: one for each orientation
...
class S2(nn.Module):
    ...
    def forward(self, c1_outputs):
        ...
        conv_output = conv_output.view(-1, self.num_orientations, self.num_patches, conv_output_size, conv_output_size)
