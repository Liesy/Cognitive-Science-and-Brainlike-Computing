...
class ReconstractionTICA(chainer.Chain):
    ...
    def __init__(self):
        ...
        with self.init_scope():
            self.f = L.Linear(1 * 32 * 32, 1 * 22 * 22)
            self.p = F.average_pooling_2d
            self.g = L.Linear(1 * 22 * 22, 1 * 32 * 32)
            self.mask = cp.zeros((1, 22, 22, 1, 32, 32), dtype=float)
            for dy in range(22):
                for dx in range(22):
                    self.mask[0, dy, dx, 0, dy:dy + 11, dx:dx + 11] = 1.0
            self.mask = self.mask.reshape((1 * 22 * 22, 1 * 32 * 32))
